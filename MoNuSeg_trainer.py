...
from dataloader.dataset_monuseg import monuseg_dataset, RandomGenerator
...
def train(
    ...
    for i_batch, sampled_batch in enumerate(trainloader):
        ...
        if iter_num % 8 == 0:
            logging.info(
                "epoch : %d, iteration : %d, train loss : %f, train loss_ce: %f, train loss_dice: %f, train dice : %f"
                % (
                    epoch_num,
                    iter_num,
                    loss.item(),
    ...
    logging.info(
        ...
        % (epoch_num, mean_loss, mean_loss_ce, mean_dice)
    )
...
def validation(
    model, writer, dice_weight, ce_weight, args, epoch_num, valloader, snapshot_path
):
    model.eval()
    ...
    with torch.no_grad():
        for i_batch, sampled_batch in enumerate(valloader):
            ...
            if iter_num % 2 == 0:
                logging.info(
                    "epoch : %d, iteration : %d, val loss : %f, val loss_ce: %f, val loss_dice: %f, val dice : %f"
                    % (
                        epoch_num,
                        iter_num,
                        loss.item(),
...
def run_main(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        if epoch_num > 200 and epoch_num % 5 == 0:
            ...
            if val_dice > highest_val_dice:
                ...
                torch.save(model.state_dict(), save_mode_path)
                logging.info(
                    "save best model to {} at epoch {}, val dice: {}".format(
                        save_mode_path, epo_val, val_dice
                    )
                )
    ...
    logging.info(
        "highest train dice: %f at epoch %d, highest val dice : %f at epoch %d"
