...
from dataloader.dataset_monuseg import monuseg_dataset
from utils.MoNuSeg_utils import test_single_volume
...
parser.add_argument(
    "--batch_size", type=int, default=1, help="batch_size per gpu"  # 24
)
parser.add_argument(
    "--img_size",
    type=int,
...
parser.add_argument(
    "--base_lr", type=float, default=1e-2, help="segmentation network learning rate"
)  # 0.01
...
def inference(args, model0, model1, model2, model3, model4, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        metric_i, index_i, binary_metric_i, time_i = test_single_volume(
            image,
            label,
            model0,
            model1,
            ...
            patch_size=args.img_size,  # maybe changed into the x and y dimensions
        ...
        logging.info(
            ...
            % (i_batch, case_name, mean_dice, mean_hd)
        )
        logging.info(
            "idx: %d, case: %s, binary dice: %f, binary hd95: %f"
            % (i_batch, case_name, binary_metric_i[0], binary_metric_i[1])
        )
    ...
    for i in range(1, args.num_classes):  # 从1开始
        ...
        logging.info(
            ...
            % (i, metric_list[i][0], metric_list[i][1])
        )
    ...
    logging.info(
        "Testing performance on classes: mean_dice : %f mean_hd95 : %f"
        % (performance, mean_hd95)
    )

    logging.info(
        "Testing performance on classes: binary mean_dice : %f binary mean_hd95 : %f"
        % (binary_metric_list[0], binary_metric_list[1])
    )
    ave_dice = ave_dice / len(db_test)
    ave_hd = ave_hd / len(db_test)
    logging.info(
        "Testing performance on cases: mean_dice : %f mean_hd95 : %f"
        % (ave_dice, ave_hd)
    )
...
if __name__ == "__main__":
    ...
    net0 = network(
        in_channel=3, out_channel=args.num_classes, training=False, config=config_vit
    ).cuda()
    net1 = network(
        in_channel=3, out_channel=args.num_classes, training=False, config=config_vit
    ).cuda()
    ...
    print(snapshot0)
    if not os.path.exists(snapshot0):
        snapshot0 = snapshot0.replace("best_model", "epoch_" + str(args.max_epochs))
    print(snapshot0)
